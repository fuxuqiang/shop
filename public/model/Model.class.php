...
class Model {
	...
	private $table;
	private $ini = array('where'=>null, 'limit'=>null);
	private $q = array('where'=>null, 'limit'=>null);
	private $db;

	public function __construct($table) {
		$pre = defined('TABLE_PREFIX')? TABLE_PREFIX:'';
		$this->table = $pre.strtolower(preg_replace('/([A-Z])/', '_$1', lcfirst($table)));
	...
	public function where($where) {
		if ($where) {
			...
			if (is_array($where)) {
				...
				$where = preg_replace('/^\s*(\w+)\s*=(^\d)/', "`$1`='$2'", $where);
	...
	public function fetchAll($fields=null) {
		...
		$q = "SELECT $fields FROM `$this->table` {$this->q['where']} {$this->q['limit']}";	  
		return $this->query($q);
	}

	public function fetch($fields=null) {
		$fields = $this->_fields($fields);
		$q = "SELECT $fields FROM `{$this->table}` {$this->q['where']}";
		$this->q = $this->ini;
	...
	public function update($data) {
		...
		// 执行查询 
		$stmt = $this->db->query(
			"UPDATE `$this->table` $q {$this->q['where']}",
			$data
		);
		// 返回结果 
		if ($stmt) return true;
